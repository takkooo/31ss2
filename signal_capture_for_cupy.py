...
# import matplotlib
# import scipy
from scipy import signal as signal_cpu
...
class SignalCapture():
    ...
    def priori_search(self, raw_data, packet_type, freq_band_idx, zc_root, freq_offset):
        ...
        if np.max(tmp) > tmp.mean() * 2:
            peaks, _ = signal_cpu.find_peaks(tmp, height=np.max(tmp) / np.sqrt(2), distance=len(zc_samples))
            if len(peaks) == 1:
...
#            for i in range(len(raw_data)):
            ...
            for u in [1, Ncarriers - 1, (1 + Ncarriers) // 2]:
                ...
                for ffo in [-20e6, -10e6, 0, 10e6, 20e6]:
                    ...
                    if np.max(tmp) > tmp.mean() * 2:
                        tmp_cpu = tmp.get()
                        peaks, _ = signal_cpu.find_peaks(tmp_cpu, height=cp.max(tmp_cpu) / cp.sqrt(2), distance=len(zc_samples))
